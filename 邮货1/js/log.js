...
$(document).ready(function() {
	$("#submit").click(function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				alert("登录成功！！！");
				$("#sub_log a:eq(0)").css("display","none");
				$("#sub_log a:eq(1)").css("display","none");
				$("#sub_log a:eq(2)").css("display","block");
				$("#usersname").html("你好！" + $("#input_username").val() + "同学");
				}  else {
					alert("密码或用户名错误！！！请仔细检查")
				}
	...
	$(".exit").click(function() {
		$("#sub_log a:eq(0)").css("display","inline-block");
		$("#sub_log a:eq(1)").css("display","inline-block");
		$("#sub_log a:eq(2)").css("display","none");
		$("#usersname").html("");
	});
});
